(ns shopee.main
  "Kotodama WASM entrypoint for the Shopee clean-room actor (L4) — Clojure port.
  ...
  facts (`shopee.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "shopee")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"shopee.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "shopee-compat" :tier tier :entities entities} 200])
...
(defn start! [] :shopee-compat/ready)
